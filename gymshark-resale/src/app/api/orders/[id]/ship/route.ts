...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (order.status !== "paid") return NextResponse.json({ error: `Ordre er i status '${order.status}', kan ikke markere som sendt` }, { status: 400 });
      ...
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          html: `<div style="font-family:-apple-system,sans-serif;color:#1c1917;max-width:560px">
            ...
            <p style="margin:0 0 12px;font-size:14px;color:#57534e">Selger har markert <strong>${itemTitle}</strong> som sendt.${body.tracking_info ? `<br><br>Sporingsinformasjon: <strong>${body.tracking_info}</strong>` : ""}</p>
            <p style="margin:0 0 16px;font-size:14px;color:#57534e">Når du har mottatt varen, gå til dine ordre og bekreft mottak. Du har 48 timer på å bekrefte eller melde problem — etter det frigjøres betalingen automatisk til selger.</p>
