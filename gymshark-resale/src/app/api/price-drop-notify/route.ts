...
export async function POST(req: Request) {
  ...
  if (payload.type !== "UPDATE" || payload.table !== "items")
    return NextResponse.json({ ok: true, skipped: true });

  const oldItem = payload.old_record;
  const newItem = payload.record;

  // Only proceed if price actually dropped
  if (!oldItem?.price || !newItem?.price || newItem.price >= oldItem.price)
    return NextResponse.json({ ok: true, skipped: "no price drop" });

  const admin = createClient(SUPABASE_URL, SERVICE_ROLE_KEY, { auth: { persistSession: false } });

  // Find users who favorited this item when price was higher
  const { data: favs } = await admin
    .from("favorites")
    .select("user_id, price_when_favorited")
    .eq("item_id", newItem.id)
    .neq("user_id", newItem.seller_id);

  if (!favs?.length) return NextResponse.json({ ok: true, skipped: "no favorites" });

  const eligible = favs.filter(
    (f) => f.price_when_favorited && newItem.price < f.price_when_favorited,
  );
  ...
  for (const fav of eligible) {
    const { data: userData } = await admin.auth.admin.getUserById(fav.user_id);
    ...
    await sendEmail(email, subject, `
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#1c1917;max-width:560px">
        ...
        <div style="background:#f5f5f4;padding:16px;border-radius:12px;margin-bottom:16px">
          <p style="margin:0 0 4px;font-size:13px;color:#78716c;text-decoration:line-through">${oldPrice} kr</p>
          <p style="margin:0;font-size:22px;font-weight:700;color:#16a34a">${newPrice} kr</p>
        </div>
